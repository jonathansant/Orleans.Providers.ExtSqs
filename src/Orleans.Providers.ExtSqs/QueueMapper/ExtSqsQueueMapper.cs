...
{
	internal class ExtSqsQueueMapper : IConsistentRingStreamQueueMapper
	{
		private readonly ILogger _logger;
		private readonly IReadOnlyDictionary<string, HashRing<InternalQueueId>> _queueMap;

		public ExtSqsQueueMapper(IEnumerable<SqsQueueProperties> queues, ILogger logger)
		{
			_logger = logger;
		...
		public QueueId GetQueueForStream(Guid streamGuid, string streamNamespace)
		{
			...
			{
				_logger.LogError("No queue for namespace: {namespace", streamNamespace);
		...
		private static IDictionary<string, HashRing<InternalQueueId>> CreateQueueMap(
			IEnumerable<SqsQueueProperties> queueProps)
		{
			return queueProps
				.Select(props => new QueueMeta(props.Name))
				.GroupBy(meta => meta.Namespace)
				.ToDictionary(
					...
					grouping =>
					{
						...
						return new HashRing<InternalQueueId>(grouping.Select((meta, iteration) =>
						{
							var uniformHashCode = (uint) 0;

							if (ringSize == 1)
							{
								return new InternalQueueId(meta.QueueName, meta.GeneratedId, uniformHashCode);
							}
		...
		private sealed class InternalQueueId : IRingIdentifier<InternalQueueId>
		{
			...
			{
				return this.Equals(obj as InternalQueueId);
			}
