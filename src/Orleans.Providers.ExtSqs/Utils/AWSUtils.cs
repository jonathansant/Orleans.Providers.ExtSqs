...
// Copied from Orleans Solution: https://github.com/dotnet/orleans/blob/master/src/OrleansAWSUtils/AWSUtils.cs.
...
namespace Orleans.Providers.ExtSqs.Utils
{
	...
	internal static class AwsUtils
	{
		...
		{
			if (key.Length >= 2048)
				throw new ArgumentException(string.Format("Key length {0} is too long to be an DynamoDB partition key. Key={1}",
					key.Length, key));
		...
		{
			if (key.Length >= 1024)
				throw new ArgumentException(string.Format("Key length {0} is too long to be an DynamoDB row key. Key={1}",
					key.Length, key));
